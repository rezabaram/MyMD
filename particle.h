...
class CParticle{
	...
	template<GType shapeType>
	explicit CParticle(const GeomObject<shapeType> &_shape):shape(new GeomObject<shapeType>(_shape)),  id(-1),  forces(vec(0.0)), state(ready_to_go),vlist(this),vlistold(this){init();}
	~CParticle(){
		delete shape;
		}
